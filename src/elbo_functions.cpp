...
void logPriorW( double &logW, arma::mat invTau, int JD, arma::mat W2, int J, 
	...
	arma::mat tmpExp = W2 % ( invTau ); 
	tmpExp *= invsigma;
			
	if( SVS ){
		logvar += (1.0 - incProbs) * log(v0); 
	...
	if( !globalvar ){ 															// Local prior variances
		if( priorvar == "jeffrey" ){
			...
			if( any(gammatau <= 0.0) ){
				for( int j=0; j<J; j++ ){
					for( d=0; d<D; d++ ){
						logtau += invgamd( invTau(j,d), logvar(j,d), alphatau(d), betatau(d) );
						hTau += invgamh( alphatau(d) + 0.5, exp( f( j,d ) ) ) ;
					}
				}
			}else{
				if( hypertype == "common" ){
					...
					for( d=0; d<D; d++ ){
						for( int j=0; j<J; j++ ){
							logtau += invgamd( invTau(j,d), logvar(j,d), alphatau(0), btmp );
							hTau += invgamh( alphatau(0)+0.5, exp( f( j,d ) ) ) ;                
						}
					}
					...
					for( d=0; d<D; d++ ){
						...
						for( int j=0; j<J; j++ ){
							logtau += invgamd( invTau(j,d), logvar(j,d), alphatau(d), btmp );
							hTau += invgamh( alphatau(d)+0.5, exp( f( j,d ) ) ) ;                
						}
					}
				}else{
		...
		if( priorvar == "jeffrey" ){
			...
			if( any( gammatau <= 0.0 ) ){									    // Fixed Hyperparameters
				for( d=0; d<D; d++ ){
					logtau += invgamd( invTau(0,d), logvar(0,d), alphatau(d), betatau(d) );
					hTau += invgamh( alphatau(d)+double(J)*0.5, exp( f( 0,d ) ) ) ;
				}
				...
				for( d=0; d<D; d++ ){
					...
					for( int j=0; j<J; j++ ){
						logtau += invgamd( invTau(0,d), logvar(0,d), alphatau(d), btmp );
						hTau += invgamh( alphatau(d)+(double(J)*0.5), exp( f( 0,d ) ) ) ;                
					}
				}
			}
